...
csspssfile_name <- "BCI_patchtype"
types <- c("High","gap","recent.gap","former.gap")
# types <- c("former.gap")

ed2in_files <- c("ED2IN")
ed2in_files <- c("ED2IN_crownmod")

TREEFALL_DISTURBANCE_RATES <- 0
...
pft_lowers <- c(stomatal_slope = 5,
                ...
                stoma_psi_b = 50,
                clumping_factor = 0.4,
                seedling_mortality = 0.5,
                seed_rain = 0.1/12*0.5,
                repro_min_h = 0,
                r_fract = 0.5,
                mort1 = 0., # 0.5
                mort3 = 0.0, # 0.
...
for (iens in seq(1,ensemble.size)){
  ...
  run_name <- paste0("Ensemble.crownmod.",iens)
  ...
  for (itype in seq(1,length(types))){
    ...
    ed2in_scenar$IYEARZ <- 2017

    ed2in_scenar$IQOUTPUT <- 3
    ed2in_scenar$ITOUTPUT <- 0
    ed2in_scenar$TREEFALL_DISTURBANCE_RATE <- TREEFALL_DISTURBANCE_RATES

    ed2in_scenar$FFILOUT <- file.path(out_ref,"analy",paste0("BCI.",types[itype]))
    ed2in_scenar$SFILOUT <- file.path(out_ref,"histo",paste0("BCI.",types[itype]))

    ed2in_scenar$SFILIN <- file.path(ICdir,paste0(csspssfile_name,"_",types[itype],"_mod"))

    write_ed2in(ed2in_scenar,filename = file.path(run_ref,paste0("ED2IN_",types[itype])))

    if (isimu == 1){
      isfirstjob = TRUE
      dir_joblauncher = run_ref
      list_dir[[paste0(run_name,"_",types[itype])]] = run_ref
    } else{
      isfirstjob = FALSE
    }

    # job.sh
    write_joblauncher(file =  file.path(dir_joblauncher,"job_BCI_patch.sh"),
                      ...
                      ED2IN = paste0("ED2IN_",types[itype]),
...
dumb <- write_bash_submission(file = file.path(rundir,"all_jobs.sh"),
                              list_files = list_dir,
                              job_name = "job_BCI_patch.sh")

# scp /home/femeunier/Documents/projects/LianaBCI/scripts/generate_ensemble_LianaBCI_patch_crownmod.R hpc:/data/gent/vo/000/gvo00074/felicien/R
